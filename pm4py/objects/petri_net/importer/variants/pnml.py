'''
    ...
    - added datetime to the imports
    - modified (the older version of) import_net_from_xml_object function -> import_net_from_xml_object_mod
        problem with the original function: it supports DPN, but in a different way 
            + it doesn't give initial values to the variables
...
from pm4py.objects.petri_net.obj import PetriNet, DataPetriNet, Marking, ResetNet, InhibitorNet, ResetInhibitorNet
...
def import_net(input_file_path, parameters=None):
    ...
    return import_net_from_xml_object_mod(root, parameters=parameters)
...
def import_net_from_xml_object_mod(root, parameters=None):
    """
    Import a Petri net from an etree XML object
    ...
    DPN = True if "type" in parameters and parameters["type"] == "DPN" else False

    if DPN:
        net = DataPetriNet('imported_' + str(time.time()))
    else:
        net = PetriNet('imported_' + str(time.time()))
    ...
    if nett is not None:
        for child in nett:
            ...
            if "finalmarkings" in child.tag:
                finalmarkings = child
            if DPN and "variables" in child.tag:
                variables = child
                    ...
                    if child2.tag.endswith('graphics'):
                        for child3 in child2:
                            if child3.tag.endswith('position'):
                                ...
                                dimension_Y = float(child3.get("y"))
                if DPN:
                    places_dict[place_id] = DataPetriNet.Place(place_id)
                else:
                    places_dict[place_id] = PetriNet.Place(place_id)
    ...
    if page is not None:
        for child in page:
            if child.tag.endswith("transition"):
                ...
                for child2 in child:
                    ...
                    if child2.tag.endswith("toolspecific"):
                        ...
                        if "ProM" in tool:
                            ...
                            random_variable.set_priority(priority)
                            random_variable.set_weight(weight)
                    if DPN and child2.tag.endswith("writeVariable"):
                        trans_write_vars.add(child2.text)
                ...
                if DPN:
                    trans_dict[trans_name] = DataPetriNet.Transition(name=trans_name, label=trans_label, guard=trans_guard, write_vars=trans_write_vars)
                else:
                    trans_dict[trans_name] = PetriNet.Transition(name=trans_name, label=trans_label)
    ...
    java_to_python_type = {"java.lang.Boolean": bool, "java.util.Date": datetime, "java.lang.Double": float, "java.lang.Long": int, "java.lang.String": str}

    if DPN and variables is not None:
        for child in variables:
            var_java_type = child.get("type")
            var_initial_value = child.get("initialValue")
            if var_java_type in java_to_python_type:
                var_type = java_to_python_type[var_java_type]
            else:
                raise Exception("Ivalid variable type in the pnml file: "+var_java_type)
            for child2 in child:
                if child2.tag.endswith("name"):
                    var_name = child2.text
                    vars_dict[var_name] = DataPetriNet.Variable(name=var_name, dom=var_type, init_val=var_initial_value)
                    net.variables.add(vars_dict[var_name])
