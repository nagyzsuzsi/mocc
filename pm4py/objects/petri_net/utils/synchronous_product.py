'''
    ...
    - added DEFAULT_NAME_KEY to the imports
    - added extend_trace_net_of_synchronous_product_net function
    Author: ...
'''
...
def extend_trace_net_of_synchronous_product_net(sync_net, event, sync_fm, skip, activity_key=DEFAULT_NAME_KEY):
    ...
    # add new transition
    t = PetriNet.Transition(('t' + new_transition_index, skip), (event[activity_key], skip))
    sync_net.transitions.add(t)
    add_arc_from_to(last_place_from_trace_net_part, t, sync_net)

    # add new place
    p = PetriNet.Place(('p_' + str(int(new_transition_index) + 1), skip))
    sync_net.places.add(p)
    add_arc_from_to(t, p, sync_net)
    ...
    for t2 in sync_net.transitions.copy():
        if transition_from_synchronous_product_net_belongs_to_process_net_part(t2) and \
            ...
            sync = PetriNet.Transition((t.name[0], t2.name[1]), (t.label[0], t2.label[1]))
            sync_net.transitions.add(sync)
            for a in t.in_arcs:
                add_arc_from_to(a.source, sync, sync_net)
            for a in t2.in_arcs:
                add_arc_from_to(a.source, sync, sync_net)
            for a in t.out_arcs:
                add_arc_from_to(sync, a.target, sync_net)
            for a in t2.out_arcs:
                add_arc_from_to(sync, a.target, sync_net)
